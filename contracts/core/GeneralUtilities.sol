...
library ReflectionUtilities {
    ...
    function delegateCall(address subject, bytes memory inputData) internal returns(bytes memory returnData) {
        bool result;
        (result, returnData) = subject.delegatecall(inputData);
        if(!result) {
            assembly {
                revert(add(returnData, 0x20), mload(returnData))
            }
...
library BytesUtilities {
    ...
    function asBytes32(bytes memory b) internal pure returns(bytes32) {
        return bytes32(asUint256(b));
    }

    function asBytes32Array(bytes memory b) internal pure returns(bytes32[] memory callResult) {
        if(b.length > 0) {
...
library Require {
    function _require(bool expression, uint256 errorCode, bytes memory payload) internal pure {
        if(!expression) {
            revert(string(abi.encodePacked(Uint256Utilities.toString(errorCode), payload.length == 0 ? "" : ": ", payload)));
        }
    }

    function _require(bool expression, uint256 errorCode) internal pure {
        _require(expression, errorCode, "");
    }
}

library SaltDependantContractUtilities {

    function create2(uint256 value, bytes memory payload, bytes32 salt) internal returns (address productAddress, bytes32 initHash) {
        assembly {
            productAddress := create2(value, add(0x20, payload), mload(payload), salt)
            switch extcodesize(productAddress) case 0 {
                invalid()
            }
        }
        require(productAddress == retrieveSaltDependantAddress(address(this), salt, initHash = keccak256(payload)), "error");
    }

    function retrieveSaltDependantAddress(address creator, bytes32 salt, bytes32 initHash) internal pure returns (address addr) {
